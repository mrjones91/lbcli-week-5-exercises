# What is the address of the P2SH script wrapping the witness redeem script in this transaction
# transaction=""
transaction=""
scriptsig=$(bitcoin-cli -regtest decoderawtransaction $transaction | jq -r '.vin[].scriptSig.asm' | cut -d ' ' -f4)
bitcoin-cli -named -regtest decodescript hexstring=$scriptsig  | jq -r '.segwit.["p2sh-segwit"]'


# chapter 10 notes
# integer=1546288031
# echo $(. ../tools/int2le.sh $integer)
# hex=$(printf '%08x\n' $integer | sed 's/^\(00\)*//')
# echo $hex
# hexfirst=$(echo $hex | cut -c1)
# [[ 0x$hexfirst -gt 0x7 ]] && hex="00"$hex
# lehex=$(echo $hex | tac -rs .. | echo "$(tr -d '\n')")
# echo $lehex
# echo -n $lehex | wc -c | awk '{print $1/2}'

# neglehex=$(printf '%x\n' $((0x$lehex | 0x80)))
# echo $neglehex

